...
st.set_page_config(page_title="RFM Analysis Dashboard",
                   page_icon="📊",
                   layout="wide")
...
# Function to generate customer segment trends
def plot_segment_trends(df):
    df['Date'] = pd.to_datetime(df['Date'])
    segment_by_month = df.groupby([pd.Grouper(key='Date', freq='M'), 'Segment']).size().unstack(fill_value=0)
    fig = px.line(segment_by_month, 
                  title='Customer Segment Trends Over Time',
                  labels={'value': 'Number of Customers', 'Date': 'Month'})
    return fig

# Function to generate RFM distribution
def plot_rfm_distribution(df):
    fig = go.Figure()
    metrics = ['Recency', 'Frequency', 'Monetary']
    for metric in metrics:
        fig.add_trace(go.Box(y=df[metric], name=metric))
    fig.update_layout(title='Distribution of RFM Metrics')
    return fig
...
def auth_page():
    ...
    with tab1:
        ...
        st.markdown("<h2 class='auth-header'>Login</h2>",
                    unsafe_allow_html=True)
    ...
    with tab2:
        st.markdown("<div class='tab-content'>", unsafe_allow_html=True)
        st.markdown("<h2 class='auth-header'>Register</h2>",
                    unsafe_allow_html=True)
        ...
        reg_pass = st.text_input("Password",
                                 type="password",
                                 key="reg_password")
        reg_confirm_pass = st.text_input("Confirm Password",
                                         type="password",
...
def main():
    ...
    uploaded_file = st.sidebar.file_uploader("Upload your customer data", type=["csv", "xlsx", "xls"])
    ...
    * **Monetary**: Total spending
    ...
    filtered_df = df[(df['Date'] >= pd.to_datetime(date_range[0]))
                     ...
                     (df['Total'].between(transaction_amount[0],
    ...
    tab1, tab2, tab3, tab4 = st.tabs(
        ["Dashboard", "Data Explorer", "Customer Segments", "About"])

    with tab1:
        # Add download button for the full report
        csv_data = export_to_csv(rfm, "rfm_analysis.csv")
        st.download_button(
            label="Download Full RFM Analysis",
            data=csv_data,
            file_name="rfm_analysis.csv",
            mime="text/csv"
        )
        
        # Display segment trends
        df_with_segment = filtered_df.merge(rfm[['Invoice ID', 'Segment']], on='Invoice ID', how='left')

        st.plotly_chart(plot_segment_trends(df_with_segment), use_container_width=True)

        
        # Display RFM distribution
        st.plotly_chart(plot_rfm_distribution(rfm), use_container_width=True)
    ...
    with tab2:
        ...
        if search_term:
            filtered_rfm = filtered_rfm[
                filtered_rfm['Invoice ID'].str.contains(search_term,
                                                        case=False)]

        if segment_filter and 'All' not in segment_filter:
            filtered_rfm = filtered_rfm[filtered_rfm['Segment'].isin(
                segment_filter)]
        ...
        try:
            ...
            st.download_button("Download Filtered RFM Data",
                               csv_data,
    ...
    with tab3:
        ...
        segment_descriptions = {
            ...
            "New Customers":
            "Customers who purchased recently but not made many purchases as yet",
        ...
        segment_metrics = segment_metrics.rename(
            columns={
                'Invoice ID': 'Count',
        ...
        # Format metrics
        segment_metrics['Avg Days Since Purchase'] = segment_metrics[
            'Avg Days Since Purchase'].round(1)
        segment_metrics['Avg Purchase Frequency'] = segment_metrics[
            'Avg Purchase Frequency'].round(1)
        segment_metrics['Avg Spend ($)'] = segment_metrics[
            'Avg Spend ($)'].round(2)
        ...
        selected_segment = st.selectbox("Select Customer Segment to Analyze",
                                        options=segment_options)

        # Check if we need to handle New Customers separately
        if selected_segment == 'New Customers' and 'New Customers' not in segment_metrics[
                'Segment'].values:
            ...
            avg_recency = np.mean(
                new_customers['Recency']) if len(new_customers) > 0 else 0
            avg_frequency = np.mean(
                new_customers['Frequency']) if len(new_customers) > 0 else 0
            avg_monetary = np.mean(
                new_customers['Monetary']) if len(new_customers) > 0 else 0

            st.markdown(f"### {selected_segment}")
            st.markdown(
                f"**Description**: {segment_descriptions.get(selected_segment, 'Customers who purchased recently but not made many purchases as yet')}"
            )
        else:
            # Display regular segment info
            segment_data = segment_metrics[segment_metrics['Segment'] ==
                                           selected_segment].iloc[0]

            st.markdown(f"### {selected_segment}")
            st.markdown(
                f"**Description**: {segment_descriptions.get(selected_segment, 'No description available')}"
            )
        ...
        if selected_segment == 'New Customers' and 'New Customers' not in segment_metrics[
            ...
            with metric_col2:
                st.metric("Avg Days Since Purchase",
                          segment_data['Avg Days Since Purchase'])

            with metric_col3:
                st.metric("Avg Purchase Frequency",
                          segment_data['Avg Purchase Frequency'])
        ...
        try:
            if selected_segment == 'New Customers' and 'New Customers' not in segment_metrics[
                 ...
                 if len(new_customers) > 0:
                     fig_box = go.Figure()
                     fig_box.add_trace(
                         go.Box(y=new_customers['Recency'], name="Recency"))
                     fig_box.add_trace(
                         go.Box(y=new_customers['Frequency'], name="Frequency"))
                     fig_box.add_trace(
                         go.Box(y=new_customers['Monetary'], name="Monetary"))
                     fig_box.update_layout(
                         title=
                         f"Distribution of RFM Metrics for {selected_segment}")
                     st.plotly_chart(fig_box, use_container_width=True)
                 else:
                     st.warning(
                         "No new customers found in the current data selection."
                     )
             else:
                 ...
                 fig_box = go.Figure()
                 fig_box.add_trace(
                     go.Box(y=segment_rfm['Recency'], name="Recency"))
                 fig_box.add_trace(
                     go.Box(y=segment_rfm['Frequency'], name="Frequency"))
                 fig_box.add_trace(
                     go.Box(y=segment_rfm['Monetary'], name="Monetary"))
                 fig_box.update_layout(
                     title=f"Distribution of RFM Metrics for {selected_segment}"
                 )
                 st.plotly_chart(fig_box, use_container_width=True)
         except Exception as e:
    ...
    "Loyal Customers": [
        ...
        "Tip 2: Offer exclusive discounts and other personalized experiences to show appreciation.",
        "Tip 3: Upgrade them to a credit card with a higher limit.",
        "Tip 4: Consider brand deals if they have influence or refer others."
    ],
    "New Customers": [
        "Tip 1: Encourage a credit card application with starter perks.",
        "Tip 2: Offer a coupon for free shipping on their first online purchase.",
        "Tip 3: Promote app downloads by explaining potential rewards or cashback.",
        "Tip 4: Get them to register for emails by highlighting benefits like welcome gifts."
    ],
    "At Risk": [
        "Tip 1: Send surveys to uncover pain points or negative experiences.",
        "Tip 2: Offer time-limited promotions or win-back coupons.",
        "Tip 3: Remind them of previously browsed or abandoned items with enticing deals.",
        "Tip 4: Follow up on their most recent purchases to ask for feedback and show you care."
    ]
}


        segment_recommendations = recommendations.get(
            selected_segment,
            ["No specific recommendations available for this segment"])
        ...
        if selected_segment == 'New Customers' and 'New Customers' not in segment_metrics[
            ...
            if len(new_customers) > 0:
                ...
                st.subheader("Export Data")
                try:
                    csv_data = new_customers.to_csv(
                        index=False).encode('utf-8')
                    st.download_button("Download New Customer Data",
                                       csv_data,
